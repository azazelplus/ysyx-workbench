...
/**
  ...
  * 2. 提供数据读写接口 (DMEM) - 给 LSU 使用 (这称为双端口设计. IMEM)
...
class PMEM extends Module {
  val io = IO(new Bundle {
    // ========== 取指接口 (IMEM) IFU<->PMEM ==========
    val imem = Flipped(new IMemIO)       // req: IFU->PMEM, resp: PMEM->IFU (Flipped 翻转方向)
  ...
  imem_read.io.raddr := io.imem.req.addr
  io.imem.resp.data := imem_read.io.rdata

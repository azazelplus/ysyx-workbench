...
// │  阶段之间通过 Decoupled (valid/ready) 握手 + Queue(1) 缓冲连接.           │
// │                                                                          │
// │  ┌─────┐   Q(1)  ┌─────┐   Q(1)  ┌─────┐   Q(1)  ┌─────┐  Q(1)  ┌────┐│
// │  │ IFU ├──→ ▪ ──→│ IDU ├──→ ▪ ──→│ EXU ├──→ ▪ ──→│ LSU ├──→▪──→│WBU ││
...
// │       - if_id_q 和 id_ex_q 被冲刷 (丢弃错误取入的指令)                  │
...
/**
  * MiniRV 顶层模块 - 多周期握手实现
  *
  * 数据通路：IFU -> Queue -> IDU -> Queue -> EXU -> Queue -> LSU -> Queue -> WBU
  * 阶段之间使用 Decoupled 握手 + Queue(1) 缓冲.
  *
  * 特点：
...
class MiniRV extends Module {
  ...
  //      - if_id_q 和 id_ex_q 接受 flush 信号, 内容被清空
  ...
  // IFU -> Queue(1) -> IDU   (跳转时冲刷)
  val if_id_q = Queue(ifu.io.out, 1, flush = Some(flush))
  ...
  // IDU -> Queue(1) -> EXU   (跳转时冲刷)
  val id_ex_q = Queue(idu.io.out, 1, flush = Some(flush))
  exu.io.in <> id_ex_q
  ...
  // pipeline_busy 控制:
  //   设置: IDU 将指令送入 id_ex_q (idu.io.out.fire)
  //   清除: WBU 提交指令 (wbu.io.inst_valid)
  //   注意: 跳转时 NOT 清除! jal/jalr 写寄存器 (如 ra), 必须等 WBU 提交后
  //   GPR 才有正确值, 这样下一条指令在 IDU 读 GPR 时才不会读到旧值.
  ...
  // 6. DMEM 连线 (LSU <-> PMEM)
  // =================================================================================
  pmem.io.dmem <> lsu.io.dmem

  // =================================================================================
  // 7. EBREAK 检测与调试 (使用 WBU 输出)
  // =================================================================================

...
class EXU extends Module {
  ...
  // CSR 写使能: 仅在有效指令时才写
  io.csr_wen   := in.is_csr && io.in.valid
  io.is_ecall  := in.is_ecall && io.in.valid
  io.is_ebreak := in.is_ebreak && io.in.valid
  io.is_mret   := in.is_mret && io.in.valid
  ...
  // 跳转控制 (仅在 valid 时有效, 避免 stale 数据产生虚假跳转)
  io.jump_en := io.in.valid && (in.is_jal || in.is_jalr || (in.is_branch && branch_taken) || in.is_ecall || in.is_ebreak || in.is_mret)

...
class LSU extends Module {
  ...
  // DMEM 通道默认值 (全部不发)
  io.dmem.ar.valid     := false.B
  io.dmem.ar.bits.addr := aligned_addr
  io.dmem.r.ready      := false.B
  io.dmem.aw.valid     := false.B
  io.dmem.aw.bits.addr := aligned_addr
  io.dmem.w.valid      := false.B
  io.dmem.w.bits.data  := wdata
  io.dmem.w.bits.mask  := wmask
  io.dmem.b.ready      := false.B
  ...
  switch(state) {
    ...
    is(s_mem_write) {
      // 同时发送 AW 和 W
      io.dmem.aw.valid     := true.B
      io.dmem.aw.bits.addr := aligned_addr
      io.dmem.w.valid      := true.B
      io.dmem.w.bits.data  := wdata
      io.dmem.w.bits.mask  := wmask

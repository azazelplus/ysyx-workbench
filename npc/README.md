...
## 2.告诉 `Abstract Machine` 有一个叫做 `riscv32-npc` 的新ARCH存在. 这样你输入`ARCH=riscv32-npc`就是合法的了

创建文件: `~/$(AM_HOME)/scripts/riscv32-npc.mk`
仿照`~/$(AM_HOME)/scripts/riscv32-nemu.mk`抄.
...
## 3. AM 驱动桩文件 (The Driver)

Makefile 配置好了，编译器会去找 npc/trm.c 等文件，但现在它们还不存在。我们需要创建最小的硬件抽象层。


## 链接脚本

我们需要从 NEMU 那里借一个链接脚本过来，或者指定通用的 riscv 脚本。
通常可以直接复制 am/src/platform/nemu/linker.ld 到 am/src/platform/npc/linker.ld。
```
cp ~/ysyx-workbench/abstract-machine/am/src/platform/nemu/linker.ld \
   ~/ysyx-workbench/abstract-machine/am/src/platform/npc/linker.ld
```

## npc侧:
完成以上步骤后，AM 侧的工作就全部完成了。

现在，如果你在 abstract-machine 目录下运行：
make ARCH=riscv32-npc ALL=dummy run

AM 会成功编译 dummy.c 和 npc/trm.c。
生成 .bin。
但是会报错：make -C .../npc run ... 失败。
因为虽然我们把文件复制到了 npc/，但你原来的 Makefile 不支持 run 目标，或者不支持接收 IMG=... 变量。你的 C++ 仿真器也不知道 0xa00003f8 是什么。

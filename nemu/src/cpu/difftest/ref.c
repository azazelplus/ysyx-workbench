...
* 
* @param dut:       DUT 寄存器缓冲区 (必须按约定格式排列)
* @param direction: DIFFTEST_TO_REF = DUT → REF (将 dut 的值写入 REF 寄存器)
*                   DIFFTEST_TO_DUT = REF → DUT (将 REF 寄存器读取到 dut)
* 
* 【寄存器布局】(RISC-V 32, sizeof = 33 * 4 = 132 bytes)
* dut 指向的内存布局:
*   offset 0x00: gpr[0]  (x0, 恒为 0)
*   offset 0x04: gpr[1]  (x1 / ra)
*   ...
*   offset 0x7C: gpr[31] (x31 / t6)
*   offset 0x80: pc
* 
* 【调用时机】
* - init_difftest(): DIFFTEST_TO_REF, 将 DUT 初始状态同步到 REF
* - difftest_step(): DIFFTEST_TO_DUT, 将 REF 执行后的状态读取出来供比较
***************************************************************************************/
__EXPORT void difftest_regcpy(void *dut, bool direction) {
  if (direction == DIFFTEST_TO_REF) {
    // DUT → REF: 将 DUT 的寄存器状态写入 nemu 的 cpu 结构体
    memcpy(&cpu, dut, DIFFTEST_REG_SIZE);
  } else {
    // REF → DUT: 将 nemu 的 cpu 寄存器状态拷贝到 dut 缓冲区
